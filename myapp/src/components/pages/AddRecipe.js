...
export default function AddRecipe() {
    ...
    const handleSubmit= async()=>{
        ...
        getUsers.forEach((doc) => {
          alldoc = doc.data()
          console.log(alldoc)
          if(alldoc.uid == auth?.lastNotifiedUid){
            myUid = {name:alldoc.userName,email:alldoc.userEmail}
          }  
        ...
        try {
            if(location?.state?.id){
                ...
                .then(docRef => {
                    swal('Your recipe is updated successfully')
                ...
                if(auth?.lastNotifiedUid){
                    ...
                    swal('login first then add recipe')
                }
            }
            } catch (e) {
